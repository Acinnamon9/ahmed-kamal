 ...
 *
 * A full-screen overlay for booking demos. It embeds an external booking widget (iframe)
 * and provides contextual benefits on a side panel.
 * Uses React Portals to render above all other UI elements in the DOM.
 */
const BookingModal: React.FC = () => {
  // useBooking hook: manages whether the modal is open or closed globally
  const { isOpen, closeBooking } = useBooking();

  // isLoading: tracks whether the iframe content has finished fetching
  const [isLoading, setIsLoading] = useState(true);

  // handleIframeLoad: hides the loading spinner once the external widget is ready
  const handleIframeLoad = () => {
    setIsLoading(false);
  };

  // Reset the loading spinner every time the modal is reopened
  React.useEffect(() => {
    if (isOpen) {
      setIsLoading(true);
    }
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-8">
          {/* 
            Backdrop Layer:
            Dimms the background content with a heavy blur.
          */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          {/* Large, soft decorative glows in the background to add depth */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.5 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            {/* left column: The Booking Widget Iframe */}
            <div className="relative flex-1 h-full min-h-[400px] md:min-h-0 bg-white">
              {/* Subtle CRT-style scanline texture */}
              ...
              {/* Secure Loading Indicator for the Widget */}
              <AnimatePresence>
                {isLoading && (
                  <motion.div
                    initial={{ opacity: 1 }}
            ...
            {/* right column: Educational / Benefits Panel */}
            <div className="w-full md:w-[380px] lg:w-[420px] p-8 md:p-10 flex flex-col justify-center border-t md:border-t-0 md:border-l border-white/10 bg-black/40 backdrop-blur-md relative overflow-hidden">
              <div className="relative z-10">
                ...
                >
                  <h3 className="text-2xl md:text-3xl font-bold text-white mb-3 tracking-tight leading-tight">
                    Deploy Your{" "}
                    <span className="text-brand-primary">AI Persona</span>
                  </h3>
                  <p className="text-white/60 text-base leading-relaxed mb-8">
                    Master your outreach with AI agents that handle phone calls
                    with human-level nuance and social media presenters that
                    maintain your presence 24/7.
                  </p>
                </motion.div>
                ...
                >
                  {[
                    {
                      icon: Smartphone,
                      text: "Phone Agents (Sub-100ms Latency)",
                    },
                    { icon: Video, text: "AI Social Media Presenters" },
                    {
                      icon: Sparkles,
                      text: "Digital Identity & Voice Cloning",
                    },
                ...
                >
                  <p className="text-white/40 text-xs italic leading-relaxed">
                    "The transition to AI voice agents was seamless. Our digital
              ...
              {/* Background Glow Overlay for the right panel */}
  ...
  // Render to Body: Ensures the modal is always in the highest possible layer of the DOM
